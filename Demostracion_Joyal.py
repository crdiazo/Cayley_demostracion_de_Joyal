...
class MainMenuScreen:
    ...
    def draw_vertices_preview(self, surface):
        ...
        # Asegurar que el área de preview no se solape con otros elementos
        preview_area = pygame.Rect(center_x - radius - vertice_rad, center_y - radius - vertice_rad, 
                                 (radius + vertice_rad) * 2, (radius + vertice_rad) * 2)
...
class TreeToFunctionMode:
    def __init__(self):
        ...
        # Botones de vértices - reorganizados para no superponer
        self.vertex_buttons = []
        self.create_vertex_buttons()
        
        # Panel de información - reposicionado
        self.info_panel_rect = pygame.Rect(20, 100, 350, 300)
        
        # Área para el gráfico - definida claramente
    ...
    def create_vertex_buttons(self):
        ...
        # Mover botones de vértices hacia arriba para no superponer con panel de progreso
        for i in range(n):
            row = i // max_per_row
            col = i % max_per_row
            x = start_x + col * 60
            y = HEIGHT - 200 + row * 50  # Movido hacia arriba
    ...
    def draw(self, surface):
        ...
        # Indicador de paso - reposicionado
...
class FunctionToTreeMode:
    def __init__(self):
        ...
        # Panel de información - reposicionado para no superponer
        self.info_panel_rect = pygame.Rect(20, 250, 350, 400)
    ...
    def draw_info_panel(self, surface):
        ...
        if self.tree_edges:
            ...
            table_title = FONT_SMALL.render("Función generada:", True, COLORS['dark'])
            ...
            # Filas de la tabla
            for i in range(min(5, n)):  # Mostrar máximo 5 filas
                v_text = FONT_SMALL.render(str(i+1), True, COLORS['dark'])
                fv_text = FONT_SMALL.render(str(self.function[i]+1 if self.function[i] is not None else "?"), True, COLORS['dark'])
                surface.blit(v_text, (header_x, table_y + 50 + i * 20))
                surface.blit(fv_text, (header_x + 40, table_y + 50 + i * 20))
            
            if n > 5:
                dots_text = FONT_SMALL.render("...", True, COLORS['dark'])
                surface.blit(dots_text, (header_x, table_y + 50 + 5 * 20))

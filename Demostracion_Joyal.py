...
# ===========================================================================
# MODO 2: FUNCIÓN → ÁRBOL (corregido y final)
# ===========================================================================
...
class FunctionToTreeMode:
    ...
    # -----------------------------
    # Compute positions inside graph_rect
    # -----------------------------
    def compute_positions(self):
        area = self.graph_rect
        cx = area.x + area.w // 2
        cy = area.y + area.h // 2
        R = min(area.w, area.h) // 2 - 70
    ...
    def _detect_cycles_ordered(self):
        ...
        self.vertices_not_in_cycles = [v for v in range(n) if v not in ordered]
        self._cycles_list = cycles

    # -----------------------------
    # BUILD TREE (FIXED SPINE VERSION)
    # -----------------------------
    def construct_tree_from_function(self):
        ...
        # --- NUEVO FIX: ordenar ciclo según vertices_in_cycles ---
        for cyc in self._cycles_list:
            if len(cyc) > 1:

                ordered = [v for v in self.vertices_in_cycles if v in cyc]
